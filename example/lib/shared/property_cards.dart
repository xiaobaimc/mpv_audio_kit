...
/// A compact labeled text field for raw-string filter params inside
/// [ExpandableFilterCard] (e.g. `compand.points`, `aecho.delays`).
///
/// [onChanged] fires on submit / focus-loss — never per keystroke — so the
/// `af` chain is rebuilt once per edit. External value changes (the reset
/// pill, an effect reload) sync into the field only while it is idle, so
/// in-progress typing is never clobbered.
class FilterParamTextField extends StatefulWidget {
  final String label;
  final String? value;
  final String defaultValue;
  final ValueChanged<String> onChanged;

  const FilterParamTextField({
    super.key,
    required this.label,
    required this.value,
    required this.defaultValue,
  ...
  @override
  State<FilterParamTextField> createState() => _FilterParamTextFieldState();
}

class _FilterParamTextFieldState extends State<FilterParamTextField> {
  late final TextEditingController _ctrl =
      TextEditingController(text: widget.value ?? '');
  late final FocusNode _focus = FocusNode()..addListener(_onFocusChange);

  @override
  void didUpdateWidget(FilterParamTextField old) {
  ...
  Widget build(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    final atDefault = (widget.value ?? '') == widget.defaultValue;
    return Padding(
      padding: const EdgeInsets.only(bottom: 8),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
            ...
            children: [
              ...
              if (!atDefault) ...[
                ...
                _filterParamResetPill(
                  cs: cs,
                  onPressed: () {
                    _ctrl.text = widget.defaultValue;
                    widget.onChanged(widget.defaultValue);
                  },
                ),
              ],
