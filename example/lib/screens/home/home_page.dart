...
class _HomePageState extends State<HomePage> {
  String? _error;
  // Log lines live in a ValueNotifier so a new mpv log entry rebuilds
  // ONLY the LogsPage subtree (via ValueListenableBuilder) — not the whole
  // shell. With `logLevel: LogLevel.debug` that is a hot path.
  final ValueNotifier<List<String>> _logs = ValueNotifier(const []);
  ...
  void _pushLog(String line) {
    if (!mounted) return;
    final next = [..._logs.value, line];
    if (next.length > 500) next.removeAt(0);
    _logs.value = next;
  }
            ...
            final constrainedPages = <Widget>[
              ...
              if (!showPinned)
                ValueListenableBuilder<List<String>>(
                  ...
                  builder: (context, logs, _) => LogsPage(
                    player: widget.player,
                    logs: logs,
                    isPinned: false,
                    onClearLogs: () => _logs.value = const [],
