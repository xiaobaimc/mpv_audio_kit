...
class _SeekerState extends State<Seeker> {
  ...
  void initState() {
    ...
    // Field updates only — no setState. build()'s StreamBuilder on
    // `position` already rebuilds on this same tick; its rebuild is
    // deferred to the next frame, by which point this listener has run,
    // so the anchors are fresh. Avoids a second rebuild per ~60 fps tick.
    _liveEdgeSub = widget.player.stream.position.listen((p) {
      if (!mounted) return;
      if (p > _liveEdge) _liveEdge = p;
      if (p > _maxBufferEnd) _maxBufferEnd = p;
      if (_dragValue != null &&
          _seekTargetMicros != null &&
    ...
    // Field update only — build()'s StreamBuilder on `bufferDuration`
    // rebuilds on this same tick (see _liveEdgeSub note above).
    _bufferSub = widget.player.stream.bufferDuration.listen((b) {
      if (!mounted) return;
      final candidate = widget.player.state.position + b;
      if (candidate > _maxBufferEnd) _maxBufferEnd = candidate;
    });
  ...
  Widget _buildSlider(
    ...
    return Column(
      children: [
        ...
        // Subscribing to `waveform` is what activates the analyzer.
        // Until the envelope is ready the stream yields `null` and the
        // builder falls back to the plain Slider unchanged. Live
        // sources also keep the plain Slider — the waveform pipeline
        // covers seekable VOD files, not the sliding live window.
        StreamBuilder<WaveformData?>(
          stream: widget.player.stream.waveform,
          builder: (context, waveSnap) {
            final wave = waveSnap.data;
            if (wave != null && !isLive && canSeek) {
              return _WaveformSeekbar(
                wave: wave,
                progress: progress,
                bufferProgress: bufferProgress,
                playedColor: cs.primary,
                unplayedColor: cs.primary.withValues(alpha: 0.18),
                bufferColor: cs.primary.withValues(alpha: 0.40),
...
/// Waveform-styled seekbar — one vertical bar per envelope bin, painted
/// from [WaveformData.min] to [WaveformData.max]. The played portion
/// (left of the playhead) is drawn in [playedColor], the rest dimmed
/// with [unplayedColor]; [bufferProgress] tints the demuxed-ahead span
/// with [bufferColor]. Tapping or dragging maps the horizontal touch
/// position to `progress = dx / width` and reports it back through the
/// seek callbacks — the parent [Seeker] turns that into `player.seek`.
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          return GestureDetector(
            ...
            onTapDown: (d) => onSeekStart(_fraction(d.localPosition.dx, width)),
            onTapUp: (d) => onSeekEnd(_fraction(d.localPosition.dx, width)),
            onHorizontalDragStart: (d) =>
                onSeekStart(_fraction(d.localPosition.dx, width)),
            onHorizontalDragUpdate: (d) =>
                onSeekUpdate(_fraction(d.localPosition.dx, width)),
            onHorizontalDragEnd: (_) => onSeekEnd(progress),
            child: CustomPaint(
              size: Size(width, AppMetrics.waveformSeekbarHeight),
...
class _WaveformPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final bins = wave.bins;
    if (bins <= 0 || size.width <= 0) return;

    final centreY = size.height / 2;
    final maxHalf = size.height / 2;
    final playedX = progress.clamp(0.0, 1.0) * size.width;
    final bufferX = bufferProgress.clamp(0.0, 1.0) * size.width;

    // Bar pitch in pixels. The envelope carries ~2000 bins; on a
    // narrow seekbar several bins collapse onto the same column —
    // that's fine, the loop overdraws and the last write wins.
    final step = size.width / bins;
    // Cap the 0.5 px bar floor at `step` — a sub-pixel step inverts clamp.
    final barWidth = (step * 0.7).clamp(step < 0.5 ? step : 0.5, step);

    final played = Paint()..color = playedColor;
    final unplayed = Paint()..color = unplayedColor;
    final buffer = Paint()..color = bufferColor;

    for (var i = 0; i < bins; i++) {
      final x = i * step;
      final lo = wave.min[i].clamp(-1.0, 1.0);
      final hi = wave.max[i].clamp(-1.0, 1.0);
      // Guarantee a hairline bar even on pure-silence bins.
      final top = centreY - hi * maxHalf;
      final bottom = centreY - lo * maxHalf;
